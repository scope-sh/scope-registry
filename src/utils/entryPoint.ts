...
function getEntryPoint0_6_0Predicate(factory: Address): (log: Log) => boolean {
  return (log) => {
    ...
    return decodedLog.args.factory.toLowerCase() === factory;
  };
}

function getEntryPoint0_7_0Predicate(factory: Address): (log: Log) => boolean {
  return (log) => {
    const decodedLog = decodeEventLog({
